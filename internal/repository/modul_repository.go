...
type ModulRepository interface {
	...
	FindAllByGuruID(guruID uint) ([]model.Modul, error)
	FindByIDAndGuruID(id uint, guruID uint) (*model.Modul, error)
	FindByID(id uint) (*model.Modul, error) // Tetap dipertahankan untuk kebutuhan internal jika ada
...
func (r *modulRepository) FindAllByGuruID(guruID uint) ([]model.Modul, error) {
	var moduls []model.Modul
	// Preload Materi dan Soal agar ketika dashboard diakses, data pendukung ikut terbawa
	err := r.db.Preload("Materi").Preload("Soal").Where("guru_id = ?", guruID).Find(&moduls).Error
	return moduls, err
}
